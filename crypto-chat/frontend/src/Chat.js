...
const Chat = ({ token, username }) => {
    ...
    try {
      ...
      if (msg.roomId) {
        ...
        if (!aesKey) {
          ...
          if (!priv) return { text: 'Private key missing' };
  ...
  return (
    <div className="chat-layout" onClick={() => { setShowReactionPickerFor(null); setHoverMessageId(null); }}>
          ...
          {isRoomMode ? (
            ...
            selectedUser ? (
              <div className="header-user">
                ...
                <div className="header-actions">
                  <button className="icon-btn" onClick={(e) => { e.stopPropagation(); setShowSearch(!showSearch); }}>🔍</button>
                  <button className="icon-btn" title="My Profile" onClick={(e) => { e.stopPropagation(); setProfileModal(username); }}>👤</button>
                </div>
              </div>
            ) : (
      ...
      {showHistory && isRoomMode && <ChatHistoryModal roomId={roomId} token={token} username={username} onClose={() => setShowHistory(false)} />}
